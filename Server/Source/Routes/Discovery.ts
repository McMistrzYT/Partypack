...
App.get("/", async (req, res) => {
    ...
    const New = {
        ...
        Songs: (await Song.find({ where: { IsDraft: false }, take: 10, order: { ReviewSubmittedAt: "DESC" } })).map(x => x.Package()),
        Priority: 100,
        Custom: false
    }
    const All = { // TEMPORARY
        ID: "all",
        Header: "All songs",
        Songs: (await Song.find({ where: { IsDraft: false }, take: 200, order: { ReviewSubmittedAt: "DESC" } })).map(x => x.Package()),
        Priority: 200,
        Custom: false
    }
